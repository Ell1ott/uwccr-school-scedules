...
import { sendCasModerationEmails } from "../_shared/moderationEmail.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!profile || profile.role !== "student") {
      ...
      return json({ error: "Only students submit CAS for approval" }, 403);
    }

    const { data: rpcData, error: casError } = await userClient.rpc(
      "pending_cas_for_token",
      { p_token: token },
    );
    if (casError) {
      log("cas_rpc_error", { error: casError.message });
      return json({ error: casError.message }, 400);
    }
    ...
    if (rows.length === 0) {
      ...
      return json({ error: "No pending CAS found for this token" }, 404);
    }

    const mailed = await sendCasModerationEmails({
      lead: `${profile.display_name} submitted a CAS that needs approval.`,
      title: rows[0].title,
    ...
    log("email_ok", { emailed: mailed.emailed, cas_id: rows[0].id });
